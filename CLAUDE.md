...
**兩份說明文件**（不是 app 的一部分，但會跟著 app 過期）

| 檔案 | 給誰 | 內容 |
|---|---|---|
| `panduan-admin.html` | 管理者 | 七章操作手冊，雙語切換，10 張手繪 SVG 示意圖 |
| `panduan-karyawan.html` | 員工 | 一張 A4 圖解，試辦時給部門主管轉貼 WhatsApp |

⚠️ **改動管理端畫面時，順手回來看一下 `panduan-admin.html` 有沒有跟著錯。**
文件不會自己更新，而錯的說明比沒有說明更糟——照著做卻做不出來的人會以為系統壞了。
`panduan-admin.html` 的頁首標了「對應 app 版本」，看的人一眼知道對不對得上，
**改版時記得一起改**（`panduan-karyawan.html` 是一張滿版 A4，沒有空間放這一行，
所以它沒有標記——改版時要自己回去看一眼）。

⚠️ 示意圖**刻意不用截圖**：管理端的真實畫面上有工號、姓名、員工寫的抱怨內容，
而這個 repo 是公開的。截圖等於把個資用圖片形式放上去，
而且**圖片裡的字搜尋不到，日後想清也找不到**。
...
| `docs/開發進度.md` | 目前做到哪裡 |
| `docs/公告文案.md` | 給員工看的公告（印尼文可直接複製）。⚠️ 開頭有一段「這個系統不是匿名的」，公告前必讀 |
