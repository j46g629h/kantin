...
const REPORT = {
  ...
  /**
   * 沒有未處理案件時，日報還要不要寄（「心跳信」）。
   *
   *   true  → 照寄，但主旨與內容都是**短版**：`OK · 系統正常 · 0 件待處理`
   *   false → 完全不寄（規格 §10.1 的原始設計）
   *
   * ⚠️ 這是一個**要隨階段調整**的開關，不是設定好就不動的常數：
   *
   *   試辦期間（349 人）→ **true**。使用者是唯一的管理者，
   *     他需要每天確認「系統還活著」。信沒來就是警報。
   *
   *   全廠上線後（11,000 人，每月 50 件以內）→ **建議改成 false**。
   *     那時候每月大約 5～10 天有事、20～25 天沒事，
   *     「沒事」的信會是「有事」的 2～3 倍。兩個月後收信的人會直接滑掉
   *     `[Kantin PCI]` 開頭的所有信——**包括那幾封真的有事的**。
   *     這正是規格 §10.1 當初不寄空信的理由。
   *
   * 📌 之所以敢在 true 的時候照寄，是因為兩種信的**主旨開頭就分得出來**
   *    （`OK ·` vs `N laporan belum diproses`），可以設信箱規則自動歸檔。
   *    如果哪天把兩者的主旨改成一樣，這個開關就不再安全了。
   *
   * ⚠️ 改這個值只要 `clasp push` 就會生效，**不必 redeploy**——
   *    排程跑的是 Head，不是部署的版本（見部署筆記「二之三」）。
   */
